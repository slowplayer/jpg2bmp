...
/*int main(int argc, char *argv[])
{
    ...
    QString str1(cc);
    qDebug()<<str1;
    qDebug()<<bf.bfType<<bf.bfSize<<bf.bfReserved1<<bf.bfReserved2<<bf.bfOffBits;
    qDebug()<<bi.biSize<<bi.biWidth<<bi.biHeight<<bi.biBitCount<<bi.biCompression<<bi.biSizeImage<<bi.biXPelsPerMeter<<
              bi.biYPelsPerMeter<<bi.biClrUsed<<bi.biClrImportant;
   // qDebug()<<(unsigned char)hImgData[0]<<(unsigned char)hImgData[1]<<(unsigned char)hImgData[2]<<(unsigned char)hImgData[3]<<(unsigned char)hImgData[4];
    QByteArray hh;
    hh=res.toHex();
    qDebug()<<hh.size();
    QString str2(hh);
